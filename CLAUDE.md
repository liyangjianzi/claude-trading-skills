...
### Developer Environment & Core Commands

This is a **scripts-only project** (`pyproject.toml` declares `packages = []`), managed with `uv` (`uv.lock` is committed). Dev tooling lives in the `dev` optional-dependency group.
...
# Lint / format (config in pyproject.toml; line-length 100, target py39)
uv run ruff check . && uv run ruff format .
uv run bandit -c pyproject.toml -r skills # security lint (skips configured in [tool.bandit])

# Validate the single-source-of-truth metadata before regenerating anything
python3 scripts/validate_skills_index.py  # validate skills-index.yaml
python3 scripts/validate_skillsets.py     # validate skillsets/*.yaml

# Regenerate derived docs from skills-index.yaml (the SSoT)
python3 scripts/generate_catalog_from_index.py   # rebuilds the API matrix in catalogs/READMEs
python3 scripts/generate_workflow_docs.py        # workflows/*.yaml -> docs
python3 scripts/generate_skillset_docs.py        # skillsets/*.yaml -> docs
```

> **`skills-index.yaml` is the single source of truth.** The API Requirements matrix (below), catalog pages, and skillset/workflow docs are *generated* from it — edit the index `integrations[]`, then run the matching `generate_*` script rather than hand-editing the derived tables.

The FMP client used by API skills is **code-generated**: `scripts/fmp_client/` holds templates (`*.py.tmpl`) and a `registry.py`; run `python3 scripts/generate_fmp_client.py` to regenerate after changing endpoints.
...
1. **Generate documentation page** (auto-gen handles the EN page + index update):
   ```bash
   python3 scripts/generate_skill_docs.py --skill <skill-name>
   ```
2. **Add to catalog category section** in `docs/en/skill-catalog.md`
3. **Add to API Requirements Matrix** in the catalog file
4. **Add to README** description in `README.md`
5. If the skill requires API keys, add to the API Requirements table in `README.md`
6. If a new category is needed, create it in the README and catalog

> **Pre-commit enforcement:** The `docs-completeness` hook blocks commits if any `skills/*/SKILL.md` exists without a corresponding `docs/en/skills/<name>.md`. Run the generate command above to fix.
...
```bash
# Generate 6-section EN page for a specific skill
# Also updates docs/en/skills/index.md automatically
python3 scripts/generate_skill_docs.py --skill <skill-name>
...
> **Skill doc ownership / drift gate:** Committed `docs/en/skills/*.md` are
...
Each skill ships its own `scripts/tests/`, and many skills define identically-named modules (`scorer.py`, `fmp_client.py`, `report_generator.py`, `calculators/`, `helpers.py`). Two pieces of machinery make a single bulk run safe:

- `pyproject.toml` sets `--import-mode=importlib` and lists every test dir in `testpaths`.
- The root `conftest.py` hooks collection/setup to evict the known *conflicting* module basenames from `sys.modules` and push the active skill's `scripts/` to the front of `sys.path`, so the right skill's module wins.

```bash
uv run python -m pytest                               # run all registered testpaths in one process
uv run python -m pytest skills/vcp-screener/scripts/tests -v   # a single skill
uv run python -m pytest skills/position-sizer/scripts/tests/test_position_sizer.py::test_name  # one test

scripts/run_all_tests.sh   # legacy: one pytest invocation per skill; honors KNOWN_SKIP
```

`scripts/run_all_tests.sh` (also the pre-push hook) is kept for backward compatibility and to isolate `KNOWN_SKIP` skills with pre-existing failures (`theme-detector`, `canslim-screener`, `pair-trade-screener`) or optional deps (`bs4`, `statsmodels`) not in the `dev` extras. Coverage gate is `fail_under = 40`.
...
| docs-completeness | local | Every `skills/*/SKILL.md` has an EN doc page |
...
## Skillsets vs. Workflows (two bundling layers)

There are two distinct, both-canonical YAML layers above individual skills — don't conflate them:

- **`skillsets/*.yaml`** — the *"what to install for this goal"* layer. A skillset is a category-scoped bundle of `required` / `recommended` / `optional` skills. A skillset `id` is **exactly** a `skills-index.yaml` category (so the Trading Skills Navigator maps a recommendation's dominant category straight to a manifest). Validate with `scripts/validate_skillsets.py`. Never create a skillset file named after a *workflow* id (e.g. no `skillsets/trade-memory-loop.yaml` — the category is `trade-memory`; `trade-memory-loop` is a workflow).
- **`workflows/*.yaml`** — the *"what to run, in order"* layer (the routines that operationalize a skillset). See below.

Both override prose: if `README.md`/`CLAUDE.md`/docs disagree with a manifest in `skillsets/` or `workflows/`, the YAML wins. Schema, error codes, and validator rules live in [`docs/dev/metadata-and-workflow-schema.md`](docs/dev/metadata-and-workflow-schema.md).
...
| [`multi-asset-opportunity-daily`](workflows/multi-asset-opportunity-daily.yaml) | daily | macro-regime-detector, theme-detector, trade-hypothesis-ideator, position-sizer, trader-memory-core |
...
- The README and documentation site are English only
